...
// lights
const ambient_light = new THREE.AmbientLight(0xFFFFFF, 5); // soft white light
scene.add(ambient_light);
GLOBAL.ambient_light = ambient_light;

const hemisphere_light = new THREE.HemisphereLight(0xF99221, 0x79440A, 5);
...
const dealwithModel = async () => {
    ...
    GLOBAL.gltf_loader.load('/workshop/invoker/models/invoker.gltf', (gltf) => {
...
const dealwithAnimations = () => {
    ...
    GLOBAL.smd_loader.load('/workshop/invoker/animations/idle.smd', (clip) => {
...
function animate() {
    ...
    const tsin = Math.abs(Math.sin(elapsedTime));
    const additionVal_6 = tsin * Math.PI / 6;
    const additionVal_4 = tsin * Math.PI / 4;

    if (elbow_L) {
        elbow_R.rotation.x += additionVal_6;
        elbow_L.rotation.y += additionVal_4;
    }
